...
NUM_INSTALLS=0
MODULE_SERVICE="dynamic_temperature_fan"
MODULE_NAME="${MODULE_SERVICE}.py"
GITHUB_USER="kdb424"
...
link_extension()
{
    ...
    ln -sf "${SRCDIR}/${MODULE_NAME}" "${KLIPPER_PATH}/klippy/extras/${MODULE_NAME}"
    echo "[OK]"
}
...
remove_service()
{
    SERVICE_FILE="${SYSTEMDDIR}/${MODULE_SERVICE}.service"
    if [ -f "$SERVICE_FILE" ]; then
        echo -n "Removing system service... "
        sudo service ${MODULE_SERVICE} stop
        sudo systemctl disable ${MODULE_SERVICE}.service
        sudo rm "$SERVICE_FILE"
        echo "[OK]"
    fi
    OLD_SERVICE_FILE="${SYSTEMDDIR}/klipper_${MODULE_SERVICE}.service"
    if [ -f "$OLD_SERVICE_FILE" ]; then
        echo -n "Removing old system service... "
        sudo service klipper_${MODULE_SERVICE} stop
        sudo systemctl disable klipper_${MODULE_SERVICE}.service
...
add_updater()
{
    ...
    update_section=$(grep -c '\[update_manager[a-z ]* ${MODULE_SERVICE}\]' $MOONRAKER_CONFIG || true)
    if [ "$update_section" -eq 0 ]; then
        echo -e "\n[update_manager ${MODULE_SERVICE}]" >> "$MOONRAKER_CONFIG"
        echo "type: git_repo" >> "$MOONRAKER_CONFIG"
        echo "path: ${SRCDIR}" >> "$MOONRAKER_CONFIG"
        echo "origin: https://github.com/${GITHUB_USER}/klipper_${MODULE_SERVICE}.git" >> "$MOONRAKER_CONFIG"
...
uinstall()
{
    if [ -f "${KLIPPER_PATH}/klippy/extras/${MODULE_NAME}" ]; then
        echo -n "Uninstalling ${MODULE_SERVICE}... "
        rm -f "${KLIPPER_PATH}/klippy/extras/${MODULE_NAME}"
        rm -f "${KLIPPER_PATH}/klippy/extras/${MODULE_NAME}c"
        echo "[OK]"
        echo "You can now remove the \"[update_manager ${MODULE_SERVICE}]\" section in your moonraker.conf and delete this directory."
        echo "You also need to remove the \"[${MODULE_SERVICE}]\" section in your Klipper configuration..."
    else
        echo -n "${KLIPPER_PATH}/klippy/extras/${MODULE_NAME} not found. Is it installed? "
        echo "[FAILED]"
    fi
}
